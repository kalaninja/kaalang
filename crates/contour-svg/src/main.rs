...
const USAGE: &str = "usage: cargo contour diagram <source.rs> --flow <name> [-o <path>]";
...
fn run(arguments: impl IntoIterator<Item = OsString>) -> Result<PathBuf, String> {
    ...
    let svg =
        contour_svg::render_source(&source, &options.flow).map_err(|error| error.to_string())?;
    let output = options
        .output
...
fn parse_arguments(arguments: impl IntoIterator<Item = OsString>) -> Result<Options, String> {
    ...
    if arguments
        .peek()
        .is_some_and(|argument| argument == "contour")
    {
        arguments.next();
    }
...
mod tests {
    ...
    fn accepts_cargo_and_direct_invocation_shapes() {
        for arguments in [
            vec!["cargo-contour", "diagram", "flow.rs", "--flow", "decide"],
            vec![
                "cargo-contour",
                "contour",
                "diagram",
