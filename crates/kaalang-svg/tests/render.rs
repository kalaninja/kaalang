...
#[test]
fn uses_kaalang_icons_without_type_captions_or_arrows() {
    let svg = render_source(SOURCE, "route").unwrap();
...
fn preserves_whitespace_in_block_and_case_labels() {
    ...
    assert!(svg.contains("aria-describedby=\"kaalang-description\""));
    assert!(svg.contains("<desc id=\"kaalang-description\">"));
}
