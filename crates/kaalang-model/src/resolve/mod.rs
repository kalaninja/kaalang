...
fn validate_wires(sources: &[Ident], blocks: &[Block]) -> Result<()> {
    ...
    for source in sources {
        if !source_names.insert(source.clone()) {
            return Err(Error::new(source.span(), "duplicate kaalang source wire"));
        }
    }
    ...
    for block in blocks {
        ...
        for input in &block.inputs {
            ...
            if !producers.contains(&input.ident) {
                return Err(Error::new(
                    input.ident.span(),
                    "a kaalang block input must name a source wire or an earlier block output",
                ));
            }
        }
        ...
        for output in &block.outputs {
            ...
            if source_names.contains(output) {
                return Err(Error::new(
                    output.span(),
                    "a kaalang block output must not reuse a source wire name",
                ));
            }
